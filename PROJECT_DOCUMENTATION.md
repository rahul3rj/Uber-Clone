...
## 12) Notes / Known Issues (Technical Debt)

- `backend/models/ride.model.js` uses `captain: { ref: 'User' }` but the captain model is `Captain`. This should be aligned.
- `/maps/*` routes use `authUser`; captains often call these endpoints too.
  - Current auth middleware does not verify that `req.user` exists after decoding, so any valid JWT signed with the same secret passes through.
- `backend/socket.js` has duplicated `call:*` handlers (same events registered twice).
- Profile images are stored as base64 strings in MongoDB; size can grow quickly.

---

## 13) Quick Checklist (What You Changed Recently)

- Moved **User GPS pin** to `/Home` so user is pinned immediately when Home opens.
- Removed **GPS from RideSelection** so it focuses on route + fare + booking.
- Updated **Captain pickup/ride distance** so it’s computed from captain’s current GPS:
  - to pickup before ride starts
  - to destination during ride